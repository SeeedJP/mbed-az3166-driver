...
OSStatus mico_rtos_get_current_thread( mico_thread_t* thread )
{
    osThreadId thread_id;
    thread_t *p;
...
OSStatus mico_rtos_start_timer( mico_timer_t* timer )
{
    ...
    ret = osTimerStart(timer->id, timer->time_ms);
    timer->state = MICO_TIMER_START;
...
OSStatus mico_rtos_deinit_timer( mico_timer_t* timer )
{
    ...
    ret = osTimerDelete(timer->id);
    timer->state = MICO_TIMER_UNUSED;
    free(timer->buf);
    timer->buf = NULL;
    timer->id = NULL;
    return ret;
...
bool mico_rtos_is_timer_running( mico_timer_t* timer )
{
    ...
    if (timer->state == MICO_TIMER_START)
        return true;
